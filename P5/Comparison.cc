...
OrderMaker ::OrderMaker(std::ifstream &fileDes)
{
	...
	while (getline(fileDes, line))
	{

		// getline( fileDes, line );

		whichAtts[numAtts] = stoi(line.c_str());
		...
		if (line == "Int")
			...
			cout << "metadata file corrupt" << endl;
		numAtts++;
	}
	// cout << "linehhhh" << endl;
	// cout << whichAtts[0] << endl;
	// cout << whichTypes[0] << endl;
}

OrderMaker *CNF::getQueryOrderMaker(OrderMaker &sortOrder)
{

	OrderMaker cnf_order;
	OrderMaker sortOrderTemp = sortOrder;
	GetSortOrders(cnf_order, sortOrderTemp);

	OrderMaker *queryOrderMaker = new OrderMaker();
	...
	for (int i = 0; i < sortOrder.numAtts; i++)
	{
		...
		{
			if ((sortOrder.whichAtts[i] == cnf_order.whichAtts[j]) && (sortOrder.whichTypes[i] == cnf_order.whichTypes[j]))
			{
				isMatched = true;
				queryOrderMaker->whichAtts[queryOrderMaker->numAtts] = sortOrderTemp.whichAtts[j];
				queryOrderMaker->whichTypes[queryOrderMaker->numAtts] = sortOrderTemp.whichTypes[j];
	...
	if (queryOrderMaker->numAtts > 0)
		return queryOrderMaker;
	else
	{
...
void OrderMaker::makeQueryOrderMaker(OrderMaker *sortOrder, const CNF &queryCNF, OrderMaker &queryOrder, OrderMaker &queryOrderForLiteral)
{
	queryOrder.numAtts = 0;
	queryOrderForLiteral.numAtts = 0;
	int attributeVal, literalVal;
	int matchFlag = 0;

	for (int i = 0; i < sortOrder->numAtts; i++)
	{
		for (int j = 0; j < queryCNF.numAnds; j++)
		{
			if (queryCNF.orLens[j] == 1)
			{
				const Comparison &cmp = queryCNF.orList[j][0];
				if ((cmp.op == Equals) && (((cmp.whichAtt1 == sortOrder->whichAtts[i]) && (cmp.operand2 == Literal)) || ((cmp.whichAtt2 == sortOrder->whichAtts[i]) && (cmp.operand1 == Literal))))
				{
					attributeVal = cmp.whichAtt1;
					literalVal = cmp.whichAtt2;
					if (cmp.operand1 == Literal)
					{
						literalVal = cmp.whichAtt1;
						attributeVal = cmp.whichAtt2;
					}
					queryOrder.whichAtts[queryOrder.numAtts] = attributeVal;
					queryOrder.whichTypes[queryOrder.numAtts] = sortOrder->whichTypes[i];
					queryOrderForLiteral.whichAtts[queryOrderForLiteral.numAtts] = literalVal;
					queryOrderForLiteral.whichTypes[queryOrderForLiteral.numAtts] = sortOrder->whichTypes[i];
					++queryOrder.numAtts,
					++queryOrderForLiteral.numAtts;
					matchFlag = 1;
				}
			}
		}
...
void OrderMaker ::Print_metadata(std::ofstream &fileDes)
{
	for (int i = 0; i < numAtts; i++)
	{
		fileDes << whichAtts[i] << endl;
		if (whichTypes[i] == Int)
			fileDes << "Int" << endl;
...
OrderMaker :: OrderMaker(Schema *schema, int index) {
	...
	for (int i = 0; i < n; i++) {
                if (atts[i].myType == Double && i == index ) {
                        whichAtts[numAtts] = i;
                        whichTypes[numAtts] = Double;
                        numAtts++;
                }
	...
	// and finally the strings
        for (int i = 0; i < n; i++) {
                if (atts[i].myType == String && i == index ) {
                        whichAtts[numAtts] = i;
                        whichTypes[numAtts] = String;
                        numAtts++;
                }
