...
public:
	...
	OrderMaker(Schema *schema);
		...
		OrderMaker(Schema *schema, int index);


	// Order Maker is created from the metadata file
	OrderMaker(std::ifstream &fileDes);

	// print to the screen
	void Print ();

	// print to metadata File
	void Print_metadata(std::ofstream &fileDes);

	// OrderMaker is built using the sortedOrder of File and CNF Order
	static void makeQueryOrderMaker(OrderMaker *sortOrder, const CNF &query,
